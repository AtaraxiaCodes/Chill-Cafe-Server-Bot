	...
	execute(message, args) {
		...
		if (getRandomInt(2) == 1) {
			...
			coins[message.author.id].coins = Math.round(
				coins[message.author.id].coins + two_perc
			);
			robbedEmbed = new discord.MessageEmbed()
				...
				.addField(
					':gun:',
					'You robbed ' +
						argsUser.user.username +
						' For 2% of their coins! (' +
						two_perc +
						')'
				)
				.addField('Chance', '1/2');
			message.channel.send(robbedEmbed).then(msg => {
				msg.delete(5000);
			});
		} else {
			...
			caughtEmbed = new discord.MessageEmbed()
				...
				.addField(
					...
					'You were robbing ' +
						argsUser.user.username +
						' and got caught by the police and you lose 2% (' +
						coinsLost +
						') of your coins!'
				)
				.addField('Chance', '1/2');
